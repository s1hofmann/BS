        ...
        while ( entry < end ) {
            ...
            switch ( entry_type ) {
                ...
                case APIC_Structure::LOCAL_APIC: {
                    ...
                    if (current_cpu != lapic_id) {
                        //DBG << "ACPI cpu numbering differs from lapic numbering" << endl;
                ...
                case APIC_Structure::IO_APIC: {
                    ...
                    //Setting IOAPICID
                    unsigned int id = conf.ioapic_id << 4;
                ...
                case APIC_Structure::SOURCE_OVERRIDE: {
                    ...
                    if(override->getBus() != 0) {
                        //DBG << "Found override for bus != ISA. Does not conform to ACPI." << endl;
                        break;
                    }
                    //DBG << "source override: remapping IRQ" << (int) override->getSource()
    ...
    while (count < mpcth->length) {
        switch (*mpt) {
            case MPCT_PROCESSOR: {
                ...
                //DBG << "found processor with lapicid " << (int) m->lapicid << " CPU "
            ...
            case MPCT_IOAPIC: {
                ...
                //DBG << "found IOAPIC with ID " << (int) conf.ioapic_id << endl;
                break;
            }
